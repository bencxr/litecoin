...
const string strMessageMagic = "GarlicCoin Signed Message:\n";
...
{
    // GarlicCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525600); // GarlicCoin: 840k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // GarlicCoin: Retarget every day
static const int64 nTargetSpacing = 1.0 * 60; // GarlicCoin: Target block per minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GarlicCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GarlicCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GarlicCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 02/Jun/2014 King Juan Carlos of Spain to Abdicate";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        if (fTestNet)
        {
            block.nTime    = 1401708942;
            block.nNonce   = 395270584;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9390f757d21c00d2d1da09becece8f4e2ec17b3b86707d101befbc775d5d852c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GarlicCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GarlicCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GarlicCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GarlicCoinMiner(CWallet *pwallet)
{
    printf("GarlicCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("garliccoin-miner");
    ...
    try { loop {
        ...
        printf("Running GarlicCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GarlicCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GarlicCoinMiner, pwallet));
}
